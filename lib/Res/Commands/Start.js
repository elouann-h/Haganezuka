...
const data = {
    ...
    execute: async (client, interaction, ctx) => {
        ...
        let [wantToStart, startMessage] = await ctx.validOrCancelDialog(`${beginText}<:color:WHITE>`, ['accept', 'decline'], ClientConfig_1.default.defaultComponentTimeout, true);
        if (wantToStart === null) {
        ...
        [raceChoice, raceMessage] = await ctx.validOrCancelDialog(`${raceChoiceText}<:color:WHITE>`, ['accept', 'decline', 'leave'], ClientConfig_1.default.defaultComponentTimeout);
        if (raceChoice === null) {
            await ctx.edit("Vous n'avez pas répondu à temps, la demande est annulée.<:color:WHITE>", raceMessage);
            return ctx.command.end();
        }
        ...
        const [wayDecision, wayPage, wayMessage] = await ctx.panelDialog('', {
            options: [
        ...
        if (wayDecision === null) {
            await ctx.edit("Vous n'avez pas répondu à temps, la demande est annulée.<:color:WHITE>", wayMessage);
            return ctx.command.end();
        }
        ...
        wayChoice = wayPage;
        await ctx.edit(`Vous êtes désormais un adepte de la Voie **« ${Content_1.wayNames[wayChoice]} »**\n\n`
            + '## Votre Art/Ensemble de techniques\nSi vous êtes un humain, vous devrez vous armez du **Style de Souffle** de votre choix.' +
            " Tandis que si vous êtes un démon, c'est un **Pouvoir Sanguinaire qu'il faudra choisir**.<:color:GREEN>`", wayMessage);
        // #### CHOIX DE L'ART ########################################################################################## //
        ...
        const [artDecision, artPage, artMessage] = await ctx.panelDialog('', {
        ...
        if (artDecision === null) {
            await ctx.edit("Vous n'avez pas répondu à temps, la demande est annulée.<:color:WHITE>", artMessage);
            return ctx.command.end();
        }
        if (artDecision.includes('leave')) {
            await ctx.edit("Vous n'avez pas choisi d'ensemble de techniques, la demande est annulée.<:color:RED>", artMessage);
            return ctx.command.end();
        }
        artChoice = artPage;
        const artInfos = artList.find((art) => art.id === artChoice);
        await ctx.edit(`Vous êtes désormais un adepte l'ensemble de techniques **« ${artInfos.name} »**.\n\n`
            + `## Nom d'Utilisateur\nIl est désormais temps de choisir votre identité. Écrivez un nom, celui que vous souhaitez utiliser durant votre aventure.`
            + `<:color:GREEN>`, artMessage);
