...
class Context {
     ...
     * @param messageToEdit The message to reply to if there is one.
     */
    async modalDialog(contentToShow, modalData, timeout, reply = false, messageToEdit) {
        const buttons = [
            new discord_js_1.ButtonBuilder().setCustomId('autodefer_modal').setStyle(v10_1.ButtonStyle.Secondary).setEmoji('📝'),
        ].concat(this.generateValidOrCancelButtons(["accept", "leave"]));
        const row = new discord_js_1.ActionRowBuilder().addComponents(buttons);
        const fullMessageData = Object.assign(this.transformMessageData(contentToShow), { components: [row] });
        const message = await this.send(fullMessageData);
        if (!message)
            return null;
        let loop = true;
    }
